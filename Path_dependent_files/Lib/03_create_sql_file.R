  ...
  file_to_create <- str_c(getwd(), "/", short_path_file_to_create)
   if(file.copy("<hard-coded-path>/Templates/template.sql", file_to_create)) {
     ...
     text_to_insert <-  str_c(
       ...
       "if(sys.nframe() == 0) { launch_sql_job(\"", short_path_file_to_create, "\") \n",
       "  } else {              run_sql_file  (\"", short_path_file_to_create, "\")}\n",
       "save_result(1, \"filename\")", "\n\n",
       "create_sql_file(\"xoooooooooooox\")" )
     line_to_replace <-
       system(
